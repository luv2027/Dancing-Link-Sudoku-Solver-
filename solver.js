...
function SolveSudoku() {
  ...
  var Sudoku = [];
  var tempRow = [];
  for (var i = 0; i < SIZE; i++) tempRow.push(0);
  for (i = 0; i < SIZE; i++) Sudoku.push(tempRow.slice());
  ...
  for (i = 1; i < 10; i++) {
    ...
    for (var j = 1; j < 10; j++) {
      ...
      if (value > 0) Sudoku[i - 1][j - 1] = value;
    }
  }
  ...
  else document.getElementById("SolvedText").innerHTML = "Solved!";
  isSolved = false;
}
...
function BuildSparseMatrix(matrix) {
  ...
  for (j = SIZE_SQUARED; j < 2 * SIZE_SQUARED; j++) {
    for (i = x; i < counter * SIZE_SQUARED; i += SIZE) matrix[i][j] = 1;
  ...
  for (j = 3 * SIZE_SQUARED; j < COL_NB; j++) {
    ...
    else if (temp % SIZE === 0) x += SIZE * (SIZE_SQRT - 1) + 1;
    else x++;
  }
}
    ...
    for (var m = 0; m < SIZE; m++) {
      if (Puzzle[n][m] > 0) {
        loop1: for (
          ...
          loop2: for (temp = Col.down; temp !== Col; temp = temp.down) {
            if (
              temp.rowID[0] === Puzzle[n][m] &&
              temp.rowID[1] - 1 == n &&
              temp.rowID[2] - 1 == m
            ) {
              break loop1;
...
function MapSolutionToGrid(grid) {
  for (var t = 0; solution[t] != null; t++) {
    grid[solution[t].rowID[1] - 1][solution[t].rowID[2] - 1] =
      solution[t].rowID[0];
  }
  for (t = 0; orig_values[t] != null; t++) {
    grid[orig_values[t].rowID[1] - 1][orig_values[t].rowID[2] - 1] =
      orig_values[t].rowID[0];
  }
}
...
function SolvedPuzzleOutput(Sudoku) {
  ...
  for (var i = 1; i < 10; i++) {
    ...
    for (var j = 1; j < 10; j++) {
      ...
      if (textField.value == "") textField.style.color = "#000000";
